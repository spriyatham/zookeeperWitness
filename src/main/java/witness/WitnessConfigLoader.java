...
/**
 * For now, it will contain only the basic configuration stuff, everything else can be added later...
 * peerType=witness
 * tickTime=2000
 * dataDir=/var/zookeeper
 * initTimeout = 5
 * pingTimeout = 4
 * server.x=<hostname>:<electionPort> Witness should only know the election port..
 * server.1=zoo1:2888
 * server.2=zoo2:2888
 * server.3=zoo3:2888
 * */
public class WitnessConfigLoader {
    private static final Logger LOG = LoggerFactory.getLogger(WitnessConfigLoader.class);
    protected File dataDir;
    protected String dataFileName;
    protected int tickTime = ZooKeeperServer.DEFAULT_TICK_TIME;
    protected int pingLimit;
    protected int initLimit;
    protected int witnessElectionPort;
    protected int witnessConnectionPort;
    //I need a QuorumServer like class and Quorum verifier like class.

    protected String configFileStr = null;
    protected String initialConfig = null;

    //BEGIN: COPY of QuorumPeerConfigVariables
    ...
    protected File dataLogDir = null;
    ...
    protected WitnessQuorumMaj quorumVerifier = null, lastSeenQuorumVerifier = null;
    protected int snapRetainCount = 3;
    protected int purgeInterval = 0;
    protected boolean syncEnabled = true;

    protected QuorumPeer.LearnerType peerType = QuorumPeer.LearnerType.WITNESS;
    ...
    //END: COPY of QuorumPeerConfigVariables


    /**
     * Parse a ZooKeeper configuration file
     * @param path the patch of the configuration file
     * @throws QuorumPeerConfig.ConfigException error processing configuration
     */
    public void parse(String path) throws QuorumPeerConfig.ConfigException {
        ...
        /**
         * Note for now removed the dynamic config file logic...assuming that I will only have a static configuration as of now..
         * */
            ...
            try {
                ...
                try {
                    ...
                    if (dynamicCfg.getProperty("version") != null) {
                        throw new QuorumPeerConfig.ConfigException("dynamic file shouldn't have version inside");
                    }
                ...
                throw new QuorumPeerConfig.ConfigException("Error processing " + dynamicConfigFileStr, e);
            } catch (IllegalArgumentException e) {
                throw new QuorumPeerConfig.ConfigException("Error processing " + dynamicConfigFileStr, e);
            }
     ...
     *
     * Note: For now copied the entire org.apache.zookeeper.server.quorum.QuorumPeerConfig#parseProperties(java.util.Properties)
     */
    public void parseProperties(Properties zkProp) throws IOException, QuorumPeerConfig.ConfigException {
        ...
        for (Map.Entry<Object, Object> entry : zkProp.entrySet()) {
            ...
            if (key.equals("dataDir")) {
            ...
            } else if(key.equals("dataFileName")) {
                dataFileName = value;
            ...
            } else if (key.equals("pingLimit")) {
                pingLimit = Integer.parseInt(value);
            } else if (key.equals("initLimit")) {
                initLimit = Integer.parseInt(value);
            } else if (key.equals("electionAlg")) {
                electionAlg = Integer.parseInt(value);
                if (electionAlg != 3) {
                    throw new QuorumPeerConfig.ConfigException("Invalid electionAlg value. Only 3 is supported.");
                }
                ...
                } else if (value.toLowerCase().equals("witness")) {
                    peerType = QuorumPeer.LearnerType.WITNESS;
                } else {
                ...
                throw new QuorumPeerConfig.ConfigException("parameter: " + key + " must be in a separate dynamic config file");
        ...
        if (clientPort != 0) {
            LOG.info("clientPort is set");
            throw new IllegalArgumentException("witness will not accept requests from client");
        }
    ...
    void setupQuorumPeerConfig(Properties prop, boolean configBackwardCompatibilityMode) throws IOException, QuorumPeerConfig.ConfigException {
     ...
     * @throws QuorumPeerConfig.ConfigException
     */
    public static WitnessQuorumMaj parseDynamicConfig(Properties dynamicConfigProp, int eAlg, boolean warnings, boolean configBackwardCompatibilityMode) throws IOException, QuorumPeerConfig.ConfigException {
        boolean isHierarchical = false;
        for (Map.Entry<Object, Object> entry : dynamicConfigProp.entrySet()) {
            ...
            if (key.startsWith("group") || key.startsWith("weight")) {
                ...
                throw new QuorumPeerConfig.ConfigException("Unrecognised parameter: " + key);
            }
        }

        WitnessQuorumMaj qv = createQuorumVerifier(dynamicConfigProp, isHierarchical);
        ...
        if (numParticipators == 0) {
            ...
            for (Witness.QuorumServer s : qv.getVotingMembers().values()) {
    ...
    private boolean parseBoolean(String key, String value) throws QuorumPeerConfig.ConfigException {
        if (value.equalsIgnoreCase("true")) {
            return true;
        } else if (value.equalsIgnoreCase("false")) {
            return false;
        } else {
            throw new QuorumPeerConfig.ConfigException("Invalid option "
                    + value
                    + " for "
                    + key
    ...
    private static WitnessQuorumMaj createQuorumVerifier(Properties dynamicConfigProp, boolean isHierarchical) throws QuorumPeerConfig.ConfigException {
        /*if (isHierarchical) {
        ...
        return new WitnessQuorumMaj(dynamicConfigProp);
    }
    ...
    public void checkValidity() throws IOException, QuorumPeerConfig.ConfigException {
        if (isDistributed()) {
            if (pingLimit == 0) {
                throw new IllegalArgumentException("pingTimeout is not set");
            }
            if (initLimit == 0) {
                throw new IllegalArgumentException("initTimeout is not set");
            }
    ...
    private void setupPeerType() {
        ...
        /*QuorumPeer.LearnerType roleByServersList = quorumVerifier.getObservingMembers().containsKey(serverId)
        ...
        //For now hardcoding it..However, it does not matter so much in the the witness itself..
        peerType = QuorumPeer.LearnerType.WITNESS;
    }
     ...
     */
    public static void writeDynamicConfig(final String dynamicConfigFilename, final WitnessQuorumMaj qv, final boolean needKeepVersion) throws IOException {
